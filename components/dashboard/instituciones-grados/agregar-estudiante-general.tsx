  ...
  useEffect(() => {
    const fetchCantidadPreguntas = async () => {
      ...
      try {
        ...
        if (data[institucion] && data[institucion][formData.grado]) {
          setTotalPreguntas(data[institucion][formData.grado]);
          setMostrarPreguntas(true);
        } else {
          toast.error(
            "No se encontró la cantidad de preguntas para este grado"
          );
          setTotalPreguntas(0);
          setMostrarPreguntas(false);
        }
        ...
        toast.error("Error al cargar la cantidad de preguntas");
        setTotalPreguntas(0);
        setMostrarPreguntas(false);
      }
    };
  ...
  const handleConfirmSubmit = async () => {
    try {
      ...
      const estudianteCompleto = {
        ...
        institucion: getInstitucionByUser(),
      };
